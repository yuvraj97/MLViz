...
def kMean(K: int,
    ...
    for _ in range(max_epochs):
        ...
        # We will get the centroid for all cluster, by there respective data points
        # and use then as new centroids.
        for i in range(n):
            group = labels[i]
        ...
        for j in range(K):
            if cardinalities[j] == 0:
                ...
                distances = np.array(
                    [np.nansum(np.sum((centroids - data[i]) ** 2, axis=1)) for i in range(n)]
                )
